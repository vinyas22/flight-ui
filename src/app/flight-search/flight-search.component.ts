...
// ...existing code...
 // 40s to 80s 
  ...
  ngOnInit() {
    this.http.get<string[]>('http://localhost:1011/api/flights/origins')
      .subscribe(data => this.originSuggestions = data);
    this.http.get<string[]>('http://localhost:1011/api/flights/destinations')
...
test() {
  ...
  searchFlights() {
    ...
    this.http.get<any[]>('http://localhost:1011/api/flights/search', { params }).subscribe(
      (res: any[]) => {
